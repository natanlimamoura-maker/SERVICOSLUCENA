...
export default function App() {
  ...
  // Compactador automático de fotos no navegador (gera WebP leve)
  ...
  const handleAddTrip = (e) => {
    ...
    if (!companyName || !freightValue) {
      alert('Preencha ao menos a Empresa e o Valor do Frete.');
      return;
    }
        ...
        <form onSubmit={handleAddTrip} className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2 text-xs text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-sky-600 file:text-white cursor-pointer"
            />
            {receiptPhoto && (
              <div className="mt-2 flex items-center gap-3">
                <img src={receiptPhoto} alt="Comprovante" className="w-12 h-12 object-cover rounded-lg border border-slate-700" />
                <span className="text-xs text-emerald-400">✓ Foto comprimida com sucesso!</span>
              </div>
            )}
          </div>
          ...
          <button
            type="submit"
            className="w-full bg-sky-600 hover:bg-sky-500 text-white font-bold p-3 rounded-xl shadow-lg transition"
          >
            Salvar Saída
          </button>
        </form>
